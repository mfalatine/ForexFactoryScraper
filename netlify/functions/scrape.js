// Netlify Function (CommonJS): Scrape ForexFactory calendar and return JSON/CSV
// Fetches the exact FF URL based on query and parses the rendered table HTML
// Times are taken verbatim from the page; date mapping avoids UTC conversion
...
function fetchText(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, {
      headers: {
        'user-agent': 'Mozilla/5.0 ForexFactoryScraperBot',
      },
    }, (res) => {
...
function toWeekParam(dateStr) {
  ...
  return `${month}${d.getDate()}.${d.getFullYear()}`; // e.g., aug19.2025
}
...
function monthIndexFromAbbr(abbr) {
  ...
  const i = arr.indexOf(String(abbr).toLowerCase());
  return i < 0 ? null : i;
}

function parseHeaderDate(label, baselineYear) {
  // Try patterns like: "Mon Jun 9" or "Jun 9"
  const m = /(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\s*(\d{1,2})/i.exec(label);
  if (m) {
    const mi = monthIndexFromAbbr(m[1]);
    if (mi != null) return new Date(baselineYear, mi, Number(m[2]));
  }
  return null;
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const start = (qs.start || '').trim(); // YYYY-MM-DD
    ...
    // Build the ForexFactory URL exactly like the site
    let url = '';
    if (dayParamRaw) {
    ...
    // Establish baseline for date mapping
    ...
    table.find('tr').each((_, el) => {
      ...
      if (dateCell.length && dateCell.text().trim()) {
        ...
        const parsed = parseHeaderDate(label, baseline.getFullYear());
        if (parsed) {
          currentIso = formatDateLocal(parsed);
        } else {
          // Fallback: advance within the current week
          dayIndex += 1;
    ...
    if (start) {
      ...
      // Keep from start through the end of its week window
      ...
      const end = new Date(baseline); end.setDate(baseline.getDate() + 6);
      const endIso = formatDateLocal(end);
